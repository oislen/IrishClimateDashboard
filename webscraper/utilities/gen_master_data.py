...
def gen_master_data(
    ...
    met_eireann_fpaths = [os.path.join(cleaned_data_dir, fname) for fname in os.listdir(cleaned_data_dir) if fname.endswith(".parquet")]
    ...
    # identify which latest year has data across all counties to avoid filtering out the most recent data for some counties which may have more recent data than others
    ...
    # if the output
    if os.path.exists(os.path.dirname(master_data_fpath)):
        ...
        raise FileNotFoundError(f"{os.path.dirname(master_data_fpath)} does not exist")